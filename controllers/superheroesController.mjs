...
export const agregarSuperheroeController = async (req, res) => {
    ...
    if (!nombreSuperHeroe || !nombreReal || !edad || !planetaOrigen || !debilidad || !poderes || !aliados || !enemigos) {
        return res.status(400).send({ mensaje: "Todos los campos son obligatorios" });
    }

    if (isNaN(edad) || edad <= 0) {
        return res.status(400).send({ mensaje: "La edad debe ser un número positivo" });
    }

    try {
        await SuperHeroRepository.crear(req.body);
...
export const editarSuperheroeController = async (req, res) => {
    const { id } = req.params;
    try {
        await SuperHeroRepository.actualizarPorId(id, req.body);
        res.redirect('/dashboard');
    } catch (error) {
